					 ...
					 success: function(data){
						 if (data.success) {
							// 成功后，置换头像图片
							 ...
							 //删除原头像，减少空间浪费
							 var old_avatarUrl = data.body;
							 var old_index=old_avatarUrl.lastIndexOf("\/");
							 var image_id=old_avatarUrl.substring(old_index+1,old_avatarUrl.length);
							 if(image_id != null) {
								 $.ajax({
									 url: 'http://localhost:8080/' + image_id,
									 type: "DELETE",
									 beforeSend: function(request) {
										 request.setRequestHeader(csrfHeader, csrfToken); // 添加  CSRF Token
									 },
